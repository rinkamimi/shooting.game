...
# ******************** 変数／定数の宣言 ********************
idx = 0 #ステージ
tmr = 0 #ゲーム内でタイミングをあわせるためのフレーム時間
restart = 0
img_size = 0.01
...
def bring_enemy(): # 敵を出す
    ...
    if idx == 1 and tmr == 1:
        set_enemy(FIELD_SIZE/2, 50, BOSS, 0, EMY_SPEED_0, BOSS_SHIELD)
        #set_enemy(FIELD_SIZE/3, 200, BOSS, 0, EMY_SPEED_0, BOSS_SHIELD)二体目


    if idx == 2 and tmr == 1:
    #ボス
        set_enemy(500, 200, BOSS, 0, EMY_SPEED_0, BOSS_SHIELD)
        set_enemy(FIELD_SIZE/3, 50, BOSS, 0, EMY_SPEED_0, BOSS_SHIELD)
    ...
    if idx == 4 and tmr == 1:
        # ボス
        set_enemy(600, 50, BOSS_MUTEKI, 0, EMY_SPEED_0, BOSS_SHIELD)
        set_enemy(50, 550, BOSS_MUTEKI, 0, EMY_SPEED_0, BOSS_SHIELD)
        # 敵：追尾タイプ
        set_enemy(80, 200, EMY_TRACKING_0, 0, EMY_LOW_SPEED, EMY_SHIELD)
        set_enemy(600, 300, EMY_TRACKING_0, 0, EMY_LOW_SPEED, EMY_SHIELD)

    if idx == 5 and tmr == 1:
        # ボス
        set_enemy(400, 60, BOSS_MUTEKI, 0, BOSS_SPEED , BOSS_SHIELD)
        # 敵：追尾タイプ
        for xpos in range(80, 960, 200):
                set_enemy(xpos, 0, EMY_TRACKING_0, 0, EMY_NORMAL_SPEED, EMY_SHIELD)
...
def set_bullet(no):
    ...
    if idx == 1 and tmr > 10:
        # ボス：プレイヤーに向かって打つ
         if tmr%40 == 0: #6フレームに一回
            rand_num = random.randint(0, 10)#0～10までの間の値をランダムで
            if rand_num <= 8:
                
                set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_1, emy_a[no], BUL_NORMAL_SPEED*0.5, BUL_SHIELD_1)
            else:
                snd_emy_bullet.play()#打つ音をplayで再生
                set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_0, emy_a[no], BUL_LOW_SPEED*0.3, BUL_SHIELD_1)


    if idx == 2 and tmr > 10:
        if emy_type[no] == BOSS:
            if tmr%80 == 0:#60フレーム毎に
                snd_emy_bullet.play()
                for a in range(0, 370, 30):
                    set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_1, a+tmr%360, BUL_LOW_SPEED*0.5, BUL_SHIELD_1)

    if idx == 3 and tmr > 10:
        if emy_type[no] == BOSS or emy_type[no] == BOSS_MUTEKI:
            if tmr%80 == 0:#100フレーム毎に
                snd_emy_bullet.play()
                for a in range(0, 370, 90):
                    set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_0, a+tmr%360, BUL_NORMAL_SPEED*0.5, BUL_SHIELD_1)


    if idx == 4 and tmr > 10:
        # ボス：プレイヤーに向かって打つ
        if emy_type[no] == BOSS or emy_type[no] == BOSS_MUTEKI:
            if tmr%20 == 0:#15フレーム毎に
                snd_emy_bullet.play()
                set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_1, emy_a[no], BUL_NORMAL_SPEED*0.5, BUL_SHIELD_1)
           
        #　敵：4方向に向かって打つ
        if emy_type[no] == EMY_FIXED:
            if tmr%80 == 0:#2.5フレーム毎に
                snd_emy_bullet.play()
                for a in range(0, 370, 90):
                    set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_0, a+tmr%360, BUL_NORMAL_SPEED*0.5, BUL_SHIELD_1)
    ...
    if idx == 5 and tmr > 10:
        if emy_type[no] == BOSS or emy_type[no] == BOSS_MUTEKI:
            if tmr%8 == 0:#フレーム毎に
                ...
                if rand_num <= 2:
                    snd_emy_bullet.play()
                    set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_0, rand_a, BUL_NORMAL_SPEED*0.5, BUL_SHIELD_1)
                elif rand_num == 3:
                    snd_emy_bullet.play()
                    set_enemy(emy_x[no], emy_y[no], BUL_TRACKING, rand_a, BUL_LOW_SPEED*0.5, BUL_SHIELD_1)
                else:
                    snd_emy_bullet.play()
                    set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_1, rand_a, BUL_NORMAL_SPEED*0.5, BUL_SHIELD_1)
                ...
                for j in range(ENEMY_MAX):
                    if emy_f[j] == True:
                        ...
                        if emy_type[j] != BOSS_MUTEKI:
                            if emy_shield[j] <= 0:
                                emy_f[j] = False
            #ここにかいてあった爆発の範囲に敵がいる場合、敵と敵の弾にダメージを消した（現時点：画像は広範囲に爆発だけど、ダメージは与えていない）
            eff_p[i] = eff_p[i] + 1
...
# ******************** プレイヤのHPの表示 ********************
#def draw_hp(sc):
    #global pl_shield,idx,tmr
    #draw_text(sc, "{}".format(str(pl_shield)), 50, 50, 110, RED)
...
def draw_title(sc, mx, my, mb):
    ...
    if 240 < mx < 720 and 30 < my < 300:
        if tmr%30 < 20: # 点滅表示
            draw_text(sc, "SELECT COURSE", SCREEN_SIZE/2, 30, 80, BLACK)
        if mb == True:  # クリック
            idx = -2
    else:
        draw_text(sc, "SELECT COURSE", SCREEN_SIZE/2, 30, 80, BLACK)
...
def draw_select_course(sc, mx, my, mb):
    ...
    for i in range(1, 6):
        if 100 < mx < 300 and i*70+60 < my < i*70+100:
            if tmr%30 < 20: # 点滅表示
                draw_text(sc, "COURSE  {}".format(i), 200, i*70+80, 20, BLACK)
            if mb == True:  # クリック
                idx = i
                tmr = 0
        else:
...
def game_init():
    ...
    pl_x = FIELD_SIZE/2
    pl_y = FIELD_SIZE - 50 #最初のプレイヤーの位置
    pl_shield = 10000
    pl_muteki = 0
