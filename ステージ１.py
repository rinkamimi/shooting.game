        ...
        # ボス：プレイヤーに向かって打つ
         if tmr%20 == 0: #20フレームに一回
            ...
            if rand_num <= 8:
                ...
                set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_0, emy_a[no], BUL_LOW_SPEED, BUL_SHIELD_1)


    if idx == 2 and tmr > 20:
        if emy_type[no] == BOSS:
            if tmr%40 == 0:#40フレーム毎に
                snd_emy_bullet.play()
                for a in range(0, 370, 30):
                    set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_1, a+tmr%360, BUL_LOW_SPEED, BUL_SHIELD_1)
        ...
        if emy_type[no] == BOSS or emy_type[no] == BOSS_MUTEKI:
            if tmr%4 == 0:#4フレーム毎に
                ...
                if rand_num <= 2:
                    ...
                    snd_emy_bullet.play()
                    set_enemy(emy_x[no], emy_y[no], BUL_TRACKING, rand_a, BUL_LOW_SPEED, BUL_SHIELD_1)
                else:
                    snd_emy_bullet.play()
                    set_enemy(emy_x[no], emy_y[no], BUL_STRAIGHT_1, rand_a, BUL_NORMAL_SPEED, BUL_SHIELD_1)
